...
function Payment(){
    ...
    useEffect(async function get(){
        let response = await axios.get('http://localhost:8081/api/cards')
  ...
  function thankyou(){
      if(savedebcard=="savecard"){
        axios({
            method:"post",
            url:"http://localhost:8081/api/newcard",
            data:{
    ...
    axios({
        method: 'delete',
        url: 'http://localhost:8081/api/selectedProductsAll/'+localStorage.getItem("username"),
      })
